...
def scan(url):
    fingerprints = {"/wp-json/wp/v2/" : "Wordpress", "/core/install.php" : "Drupal",
                ...
                "/tildacdn.com" : "Tildcdn", "/index.php?route=common/home" : "Opencart",
    ...
    for fingerprint in fingerprints.keys():
        ...
        for footprint in footprints.keys():
            if footprint in request.text:
                print(footprints.get(footprint) + ": ✅")
                found = True
        if not found:
            print(fingerprints[fingerprint] + ": ❌")
    ...
    if request.status_code == requests.codes.ok:
        ...
        print("Bitrix: Access is forbidden")
    elif request.status_code == 301:
        print("Bitrix: returned status 301")